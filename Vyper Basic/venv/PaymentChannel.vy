...
# Thực tế, quyền lợi của sender k đảm bảo. VD A gửi B 1 ether là đúng luật nhưng B lại bảo A phải gửi cho mình 2 ether cơ. Chừng nó B k close cái channel thì ether của A sẽ lock trong contract này mãi. Do đó phải có tính năng expire. Ở đây nếu 7 ngày sau mà channel k được close thì A có quyền rút hết tiền của mình ra bằng hàm dưới
@external
def cancel():
...
# Điều đặc biệt là ta dùng selfdestruct nên mỗi khi B claim money thì channel sẽ đóng lại, anh ta k thể claim nhiều lần với nhiều chữ ký được.
# A deploy trans -> gọi getHash và dùng metamask ký cho phép B rút 1 ether r gửi sig cho B -> lại muốn gửi B 1 ether nx thì A lại gọi getHash dùng metamask ký cho phép B rút 2 ether và gửi B -> giao dịch xong B có 2 signature 1 ether và 2 ether thì dương nhiên B sẽ đóng trans với sig rút 2 ether rồi là contract tự huy luôn
# => Payment Channel cũng chỉ là smart contract ez như v

# Ở trên có hàm verify là 1 hàm external là để cho B có thể kiểm tra bất cứ lúc nào rằng sig mà A gửi cho mình có đúng hay không.Vì A bịa ra 1 sig gửi cho thì chết. Đó cx là NN có cặp hàm internal và external để bên ngoài check như v